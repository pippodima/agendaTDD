...
class AgendaControllerTest {
	...
	void setUp() {
		when(transactionManager.doInTransaction(any())).thenAnswer(invocation -> {
			...
			dimartinofilippo.agenda.transaction.TransactionCode<Object> code = invocation.getArgument(0,
					dimartinofilippo.agenda.transaction.TransactionCode.class);
	...
	@Test
	void testAllToDosEmpty() {
		when(todoRepository.findAll()).thenReturn(List.of());

		List<ToDo> result = agendaController.allToDos();

		assertEquals(0, result.size());
		verify(todoView).showAllToDos(List.of());
	}
