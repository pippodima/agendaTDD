...
class ToDoSwingViewMongoIT {
	...
	void testAddButtonError() {
		...
		window.label("errorMessageLabel").requireText("same ToDo already in the agenda: Existing Task");
	}
	...
	void testDeleteButtonError() {
		...
		window.label("errorMessageLabel").requireText("ToDo doesn't exist: Non-existent task");
	}
